...
SmiTriX is a self-hosted app: you run the server, you hold the data. This file says which
...
<https://github.com/SmitroniX/SmiTriX/issues/new> and tick **"This issue is confidential"**
...
> `github.com/SmitroniX/SmiTriX/security/advisories/new` no longer resolves.
...
- **The published images** `ghcr.io/smitronix/smitrix-api` and `/web`.
...
Read this before hosting SmiTriX for anyone other than yourself.
...
- **Nothing in `./data` is encrypted.** It holds `db.json` (users, passkey public keys, push
  ...
  `secret` can mint a valid session cookie for any account. **If you host SmiTriX for other
