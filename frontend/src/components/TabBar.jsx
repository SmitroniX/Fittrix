...
export default function TabBar({ onStart }) {
  ...
  const Tab = ({ k, icon, to, label }) => (
    <button type="button" aria-label={label} className={on(k) ? 'on' : ''} onClick={() => nav(to)}>
  ...
  return (
    <nav id="tabbar" aria-label="Main Navigation">
      ...
      <button
        type="button"
        aria-label={S.active ? (cur === 'workout' ? t('Workout') : t('Resume')) : t('Start')}
        className={'start' + (S.active ? ' rec' : '') + (S.active && cur === 'workout' ? ' on' : '')}
