...
hardware can hold and it appears in the picker on the next refresh; nothing in SmiTriX pins
...
Complete its normal browser sign-in and copy the token it prints. SmiTriX never opens or handles
...
**The instance owner is responsible for their provider's terms.** SmiTriX does not interpret
...
- **Use my self-hosted SmiTriX.** Pair the phone with your instance (the same pairing flow as
