# Self-hosting SmiTriX

SmiTriX is two small containers (a web server and an API) plus a folder of your data.
...
SmiTriX signs you in with **passkeys** (WebAuthn). Browsers enforce two rules:
...
passkey prompt won't appear. To use SmiTriX from your phone you need a real HTTPS hostname.
...
Put SmiTriX behind something that terminates TLS for a hostname you control, then point it at
...
SmiTriX only needs the browser to reach it over `https://gym.example.com`.
...
The defaults assume SmiTriX is the only thing here: a service called `api` on port 3000, and nginx
...
```bash
tar czf smitrix-backup-$(date +%F).tar.gz data/
```
...
SmiTriX can push two kinds of alert to your phone/desktop, even when the app isn't open:
...
you about your pushes. SmiTriX sends your `ORIGIN` by default; set `VAPID_SUBJECT=mailto:you@example.com`
