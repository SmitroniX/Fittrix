...
application (Claude Desktop, Cursor, Cline, Continue, etc.) read your SmiTriX profile —
...
session secrets — it can only read the same `state-<uid>.json` files the SmiTriX api already
writes.
...
SMITRIX_UID=<your-uid> SMITRIX_DATA=/path/to/SmiTriX/data node src/index.js
```
...
```jsonc
{
  "mcpServers": {
    "smitrix": {
      "command": "node",
      "args": ["/absolute/path/to/SmiTriX/mcp/src/index.js"],
      "env": {
        "SMITRIX_DATA": "/absolute/path/to/SmiTriX/data",
        "SMITRIX_UID": "<your-uid>"   // optional — auto-detected if you have one profile
      }
    }
  }
}
...
Restart the client; you should see the SmiTriX tools appear with "serving profile \<name\>" on
the server's stderr.
...
AGPL-3.0-or-later, same as SmiTriX.
